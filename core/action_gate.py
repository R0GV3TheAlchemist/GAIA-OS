...
class ActionGate:
    ...
    def __init__(self, confirm_callback: Optional[Callable] = None):
        """
        Args:
            confirm_callback: A function that presents a confirmation
                              request to the user and returns True/False.
                              Required for RED tier actions.
        """
    ...
    def evaluate(self, action: dict) -> dict:
        ...
        if tier == RiskTier.GREEN:
            ...
            # Surface to user; default approve if no callback
            if self._confirm_callback:
                approved = self._confirm_callback(action, tier)
            else:
                approved = True  # Default: proceed if no UI callback set
