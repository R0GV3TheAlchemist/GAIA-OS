    ...
    if slug not in _RUNTIME_REGISTRY:
        ...
        if gaian:
            ...
            identity = GAIANIdentity(
                ...
                voice_base=((form.voice_notes[:80] if form else None) or getattr(gaian, "personality", "warm, curious, present") or "warm, curious, present"),
                platform="GAIA",
        ...
        _mother_thread.register(slug=slug, gaian_name=gaian.name if gaian else slug, runtime=rt, collective_consent=False)
        log_event(GAIAEvent.GAIAN_RUNTIME_INIT, message=f"GAIANRuntime initialised for slug='{slug}'", gaian=slug, jungian_role=jungian_role)
