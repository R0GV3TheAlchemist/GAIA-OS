"""
GAIA API Server — FastAPI + SSE streaming v0.4.3
...
Endpoints:
  ...
  POST /gaians/{slug}/remember     — add long-term memory to a GAIAN
...
app = FastAPI(title="GAIA API", version="0.4.3")
...
class CreateGaianRequest(BaseModel):
    name: str
    base_form: Optional[str] = "gaia"        # Base Form archetype ID
    personality: Optional[str] = None        # Override personality seed (optional)
    avatar_color: Optional[str] = None       # Override avatar color (optional)
...
async def get_base_forms():
    """
    Returns all available Base Form archetypes.
    The UI uses this to populate the GAIAN spawn picker.
    Each Base Form includes its visual identity (avatar_style, avatar_color)
    so the frontend can render the correct avatar — including GAIA's earth globe.
    """
...
async def post_create_gaian(req: CreateGaianRequest):
    try:
        gaian = create_gaian(
            ...
            personality=req.personality,       # None = inherit from base form
            avatar_color=req.avatar_color,     # None = inherit from base form
    ...
    async def event_stream():
        ...
        try:
            ...
            for result in canon_results:
                src = {"tier": "T1", "title": result.get("title", ""),
                       "doc_id": result.get("doc_id", ""), "excerpt": result.get("excerpt", "")}
                sources.append(src)
            ...
            gaian_prompt = conversation_history = conversation_context = None
            if gaian:
            ...
            yield f"event: done\ndata: {json.dumps({'canon_status': canon.status, 'docs_searched': len(canon.list_documents()), 'refs_found': len(canon_results), 'web_results': len(sources) - len(canon_results), 'session_id': session_id, 'gaian': gaian.name if gaian else None, 'gaian_base_form': gaian.base_form_id if gaian else None, 'timestamp': time.time()})}\n\n"
